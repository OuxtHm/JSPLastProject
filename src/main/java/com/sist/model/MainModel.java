...
/*
	...
	JSP
		1) 자바기반의 서버 사이드 웹 기술
		2) HTML 내부의 자바 코드를 첨부해서 동적 페이지 생성
	...
	5. Cookie
		...
		=> 보안이 취약하다 => 최신방문 / 장바구니
		=> 생성
			Cookie cookie = new Cookie(키, 값); => 둘 다 문자열로 되어있다
		=> 저장 기간
	...
	6. EL (화면에 데이터 출력)
		...
		${requestScope.키} => request.getAttribute("no")
			=> ${requestScope.no}
			=> ${no} => requestScop.은 생략이 가능하다
		${sessionScop.키} => session.getAttribute("id")
			=> ${sessionScope.id} 
		=> 연산자 : 숫자 / 문자 / 날짜
			산술 : +, -, *, /, %
				------------------
			...
			논리 : &&, ||
	------------------------------------------------------------------------------- Model 1 방식
	JSP = JSP : Java + HTML (JSP에서 모든 걸 처리)
	---------- 1. 확장이 어렵다
			   2. 재사용을 할 수 없다.
			   3. 보안에 취약하다 : 소스 노출
			   		*** Front-End(화면 출력용)
	...
	| 역할 분리 => 유지보수와 확장성을 높이는 설계 패턴
		MV : Model + View	
		 MVC : Model + View + Controller(자바/HTML 매칭)
	...
	**Controller
	...
	해당 Model
		| 요청값 받기
		| 데이터 베이스 연동
		| 결과 값을 request(session)에 담기 -> request.setAttribute()
	**Controller
		| Model에서 보내준 request를 가지고 온다.
	View(JSP)
		| Controller이 보내준 request를 이용하여 화면 출력
